...
public class Management {
 ...
 private Long parsePhoneFromLine(String line){
   try {  
 ...
 public Boolean checkUser(Long phone){ //returns true if user phone is in DB, false if not
     return customerRepository.checkUser(phone);
   }
 
 public List<ReturnItem> getLatestReturnDate(Long phone) 
 {
   return customerRepository.getOutstandingReturns(phone);
 ...
 public boolean createUser(Long phone){
   return customerRepository.createUser(phone);
 }
 
 public static void addRental(long phone, List <Item> rentalList)
 {
  CustomerRepository repo = new CustomerRepository();
  repo.addRental(phone, rentalList);
 ...
 {
  CustomerRepository repo = new CustomerRepository();
  repo.updateRentalStatus(phone, returnedList);
